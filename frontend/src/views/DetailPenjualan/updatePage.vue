            ...
            <div class="card-body">
              ...
              <form @submit.prevent="update">
                ...
                <div class="form-group mb-3">
                  ...
                  <select class="form-control" v-model="detail.hdr_pn">
                  ...
                  <!-- validation -->
                  <div v-if="validation.hdr_pn" class="mt-2 alert alert-danger">
                    {{ validation.hdr_pn[0] }}
                  </div>
                </div>
              <div class="mb-3">
    ...
    export default {
        ...
        setup() {
            ...
            onMounted(() => {
              //get API from Laravel Backend
              axios
                .get("http://149.28.137.82/api/cariVerifjual")
                .then((response) => {
                  //assign state posts with response data
            ...
            onMounted(() =>{
                ...
                axios
                .get(`http://149.28.137.82/api/detailjuals/${route.params.id_detail}`)
                .then((response) => {
                //assign state posts with response data
                detail.qty=response.data.data.qty,
                detail.id_barang=response.data.data.id_barang,
                detail.hdr_pn=response.data.data.hdr_pn,
            ...
            function update() {
                ...
                let hdr_pn = detail.hdr_pn
                let qty = detail.qty

                //send server with axios
                axios.put(`http://149.28.137.82/api/detailjuals/${route.params.id_detail}`, {
                        id_barang: id_barang,
                        hdr_pn: hdr_pn,
                        qty: qty
                })
