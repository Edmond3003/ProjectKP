        ...
        <div class="col-md-8">
          <div class="card">
            <div class="card-body">
              ...
              <hr>
              <router-link :to="{ name: 'cabang.create' }" class="btn btn-success mb-3">TAMBAH CABANG</router-link>
              <table class="table table-striped table-bordered">
                ...
                <tbody>
                  <tr v-for="(cabang, id) in cabangs" :key="id">
                    <td>{{ cabang.id }}</td>
                    <td>{{ cabang.nama_cabang }}</td>
                    <td>{{ cabang.alamat_cab }}</td>
                    <td>
                      <router-link
                        :to="{ name: 'cabang.edit', params: { id: cabang.id } }"
                        class="btn btn-sm btn-primary mr-1" style="margin-right: 20px;">
                        EDIT
                      </router-link>
                      <button @click.prevent="cabangDelete(cabang.id)" class="btn btn-sm btn-danger ml-1">DELETE</button>
                    </td>
                  </tr>
                </tbody>
              </table>
...
.background-image {
  ...
  export default {
    setup() {
      ...
      //method delete
      function cabangDelete(id) {
        //delete data post by ID
        axios
          .delete(`http://149.28.137.82/api/cabangs/${id}`)
          .then(() => {
            // splice posts
            const index = this.cabangs.findIndex(
              (cabang) => cabang.id === id
            );
            if (~index) {
              // if the post exists in array
              this.cabangs.splice(index, 1);
            }
          })
          .catch((error) => {
            console.log(error.response.data);
